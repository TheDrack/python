...
def save_env_file(
    ...
    """Save configuration to .env file with encrypted sensitive values"""
    print_info("\nSalvando configurações no arquivo .env...")
    print_info("Criptografando valores sensíveis com chave baseada em hardware...")
    ...
    try:
        ...
        print_success("Valores sensíveis criptografados com sucesso!")
    except Exception as e:
        print_error(f"Erro ao criptografar valores: {e}")
        return False
    ...
    if env_example_path.exists():
        with open(env_example_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                if line.startswith('GEMINI_API_KEY='):
                    env_content.append(f'GEMINI_API_KEY={encrypted_api_key}\n')
                elif line.startswith('DATABASE_URL='):
                    env_content.append(f'DATABASE_URL={encrypted_database_url}\n')
        ...
        env_content = [
            ...
            f"GEMINI_API_KEY={encrypted_api_key}\n",
            "\n",
            "# Database Settings (ENCRYPTED)\n",
            f"DATABASE_URL={encrypted_database_url}\n",
        ]
    ...
    try:
        ...
        print_warning("⚠ IMPORTANTE: Este arquivo .env está criptografado para este computador!")
        print_warning("⚠ Se movido para outro PC, será necessário executar o setup novamente.")
        return True
