        ...
        if pytest_report_path and Path(pytest_report_path).exists():
            ...
            try:
                ...
                if failed_tests:
                    ...
                    error_msg = first_failure.get('call', {}).get('longrepr', '')
                    traceback_info = first_failure.get('call', {}).get('crash', {}).get('message', '')
                    
                    # Store error details for review
                    error_details = f"Test: {first_failure.get('nodeid', 'Unknown')}\n{error_msg}\n\nTraceback:\n{traceback_info}"
                    
                    # Update issue_body with error information if not already set
                    if not issue_body:
        ...
        if pytest_passed:
            ...
            if not self.create_branch(issue_id):
                ...
                return {
                    'success': False,
                    'file': file_path,
                    'description': f'{fix_type.capitalize()} aplicada, pytest passou, mas falha ao criar branch'
                }
            ...
            if not self.commit_changes(file_path, issue_id):
                ...
                return {
                    'success': False,
                    'file': file_path,
                    'description': f'{fix_type.capitalize()} aplicada, pytest passou, mas falha ao fazer commit'
                }
            
            if not self.push_branch(issue_id):
                logger.warning("Failed to push branch")
                return {
                    'success': False,
                    'file': file_path,
                    'description': f'{fix_type.capitalize()} aplicada, pytest passou, mas falha ao fazer push'
                }
