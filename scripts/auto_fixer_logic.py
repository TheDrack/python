...
"""
    ...
    def run(self) -> int:
        ...
        if self.source == "jarvis_user":
            logger.info(f"   Mission ID: {self.mission_id or 'Not set'}")
            logger.info(f"   Session ID: {self.session_id or 'Not set'}")
            logger.info("   ℹ️  Using Jarvis identifiers for tracking")
        else:
            logger.info("   ℹ️  Direct from GitHub Actions - skipping Jarvis identifiers")
