...
class TestSystemCommands:
    ...
    def system_commands(self):
        ...
        # Mock both pyautogui and pynput for the lazy imports inside __init__
        with patch.dict("sys.modules", {"pyautogui": MagicMock(), "pynput": MagicMock(), "pynput.keyboard": MagicMock()}):
    ...
    def test_type_text(self):
        ...
        with patch.dict("sys.modules", {"pyautogui": MagicMock(), "pynput": MagicMock(), "pynput.keyboard": MagicMock()}):
            # Mock Controller after importing the modules
    ...
    def test_press_key(self, system_commands):
        """Test press_key method"""
        # Access the mocked pyautogui from the instance
        system_commands.press_key("enter")
        # Since we're using a mocked version, verify it was called
        if system_commands._pyautogui_available:
            system_commands._pyautogui.press.assert_called_once_with("enter")

    def test_hotkey(self, system_commands):
        """Test hotkey method"""
        system_commands.hotkey("ctrl", "c")
        if system_commands._pyautogui_available:
            system_commands._pyautogui.hotkey.assert_called_once_with("ctrl", "c")

    def test_click(self, system_commands):
        """Test click method"""
